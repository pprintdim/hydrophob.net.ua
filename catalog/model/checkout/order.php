...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			// плашка замовлення в спільну базу групи Hydrophob (мʼяко: CRM недоступна — пропускаємо)
			$this->load->library('group');

			try {
				$this->group->pushOrder($order_info, $order_status_id);
			} catch (\Exception $e) {
				$this->log->write('group: pushOrder ' . $e->getMessage());
			}
