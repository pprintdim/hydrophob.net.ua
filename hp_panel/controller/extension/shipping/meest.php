...
// (model extension/shipping/delivery) — тут лише статус і вартість «Meest»,
...
class ControllerExtensionShippingMeest extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$settings['shipping_delivery_meest_cost'] = $this->request->post['shipping_delivery_meest_cost'];
			$settings['shipping_delivery_meest_enabled'] = $this->request->post['shipping_delivery_meest_enabled'];
		...
		$data['breadcrumbs'] = array(
			...
			array('text' => $this->language->get('heading_title'), 'href' => $this->url->link('extension/shipping/meest', 'user_token=' . $this->session->data['user_token'], true))
		);

		$data['action'] = $this->url->link('extension/shipping/meest', 'user_token=' . $this->session->data['user_token'], true);
		$data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=shipping', true);

		$cost_key = 'shipping_delivery_meest_cost';
		$enabled_key = 'shipping_delivery_meest_enabled';
		...
		$data['sync_url'] = $this->url->link('extension/shipping/meest/syncMeest', 'user_token=' . $this->session->data['user_token'], true);
		$data['sync_param'] = 'offset';
		$data['sync_start'] = 0;
	...
	public function syncMeest() {
		...
		if (!$this->user->hasPermission('modify', 'extension/shipping/meest')) {
			...
			if ($offset == 0) {
				$this->db->query("DELETE FROM " . DB_PREFIX . "np_warehouse WHERE carrier = 'meest'");
			}

			// Meest працює у великих містах — беремо ті, де НП має від 3 точок
			$cities = $this->db->query("SELECT city FROM " . DB_PREFIX . "np_warehouse
				WHERE carrier = 'novaposhta' GROUP BY city HAVING COUNT(*) >= 3
			...
			$slice = array_slice($list, $offset, $chunk);
			$added = 0;

			foreach ($slice as $city) {
				$data = $this->httpJson('https://publicapi.meest.com/branches?' . http_build_query(array('city' => $city, 'lang' => 'ua')));

				$rows = array();

				foreach (($data['result'] ?? array()) as $item) {
					$city_ua = trim((string)($item['city']['ua'] ?? ''));

					if ($city_ua === '') {
						continue;
					}

					$type_public = $item['type_public'] ?? null;
					$branch_type = trim(is_array($type_public) ? ($type_public['ua'] ?? '') : (string)$type_public);

					if ($branch_type === '') {
						$branch_type = 'Відділення';
					}

					$number = trim((string)($item['num_showcase'] ?? ''));
					$label = $branch_type . ($number !== '' ? ' №' . $number : '');
					$is_postomat = (mb_stripos($branch_type, 'поштомат') !== false);

					$rows[] = "('meest','" . $this->db->escape($city_ua) . "','" . $this->db->escape(trim((string)($item['city']['ru'] ?? ''))) . "','"
						. ($is_postomat ? 'postomat' : 'branch') . "','" . $this->db->escape($label) . "','" . $this->db->escape($label) . "','"
						. $this->db->escape($number) . "','" . $this->db->escape(trim((string)($item['br_id'] ?? ''))) . "')";
				}
			...
			$json['added'] = $added;
			$json['done'] = (($offset + $chunk) >= count($list));
			$json['next'] = $offset + $chunk;
			$json['total'] = (int)$this->db->query("SELECT COUNT(*) AS c FROM " . DB_PREFIX . "np_warehouse WHERE carrier = 'meest'")->row['c'];
		}
