...
class ControllerExtensionModuleHpFaq extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('hp_faq', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/hp_faq', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/hp_faq', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$data['faq_description'] = $this->value('faq_description', isset($module_info) ? $module_info : array(), array());
		...
		$this->response->setOutput($this->load->view('extension/module/hp_faq', $data));
	}
	...
	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/hp_faq')) {
		...
		if ($uk && !empty($this->request->post['items'])) {
			foreach ($this->request->post['items'] as $item) {
				if (empty($item[$uk]['question'])) {
					$this->error['warning'] = $this->language->get('error_item_question');
